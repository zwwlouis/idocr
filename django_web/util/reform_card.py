...
class ReformCard:
    ...
    # template为身份证模板, target为需要识别的图像
    def find(self, template, target):
        """
        寻找图像中的身份证并变形撑满整个图片
        :param template:
        :param target:
        ...
        img1 = cv2.UMat(cv2.imread(template, 0))  # queryImage in Gray
        img1 = self.img_resize(img1, 640)
        # self.showimg(img1)
        # img1 = idocr.hist_equal(img1)
        img2 = cv2.UMat(cv2.imread(target, 0))  # trainImage in Gray
        img2 = self.img_resize(img2, 1920)
        # img2 = idocr.hist_equal(img2)
        img_org = cv2.UMat(cv2.imread(target))
...
if __name__ == "__main__":
    idfind = ReformCard()
    result = idfind.find('idcard_mask.jpg', 'testimages/13.jpg')
